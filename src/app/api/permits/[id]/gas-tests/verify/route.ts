...
/**
 * POST /api/permits/[id]/gas-tests/verify
 *
 * Bulk-verifies all pending gas tests on the permit in one action.
...
export async function POST(
  ...
  if (
    ...
    return NextResponse.json(
      {
        error:
          `Gas tests can only be verified while the permit status is draft or pending approval (current: ${permit.status})`,
      },
      { status: 400 }
    )
  }
  ...
  if (updateError) {
    console.error(
      'Failed to bulk-verify gas tests:',
      updateError
    )
    return NextResponse.json(
      {
